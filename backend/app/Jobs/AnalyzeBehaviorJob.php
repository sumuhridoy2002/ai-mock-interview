...
class AnalyzeBehaviorJob implements ShouldQueue
{
    use Queueable;
    ...
    public function handle(AiGatewayService $aiGateway): void
    {
        ...
        try {
            $result = $aiGateway->analyzeBehavior($this->videoPath, $this->question);
            ...
            Log::info('Behaviour analysis stored', [
                'answer_id'  => $this->answer->id,
                'confidence' => $result['confidence'] ?? null,
                'nervousness' => $result['nervousness'] ?? null,
            ]);
        } catch (\Throwable $e) {
            Log::error('AnalyzeBehaviorJob failed', [
                'answer_id' => $this->answer->id,
                'error'     => $e->getMessage(),
            ]);
